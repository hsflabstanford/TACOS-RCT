...
dat <- read.csv('./data/cleaned-tacos-data.csv') |> 
  ...
  income_dat <- read.csv('./data/income-data.csv') |>
    ...
    mutate(location = case_when(
      ...
      NAME == "Doña Ana County, New Mexico" ~ "Dona Ana, NM",
...
> Adding “Chick’nitas” will be most effective at reducing demand for chicken rather than meat generally. We expect declines in all categories, but concentrated in the chicken category.
...
* H5c: P(Sofritas | eggs=0 + dairy=0) + P(Chick’nitas | eggs=0 + dairy=0) + P(Veggie | * eggs=0 + dairy=0) > P(Sofritas | eggs>0 + dairy>0) + P(Chick’nitas | eggs>0 + dairy>0) * + P(Veggie | eggs>0 + dairy>0)
...
* H5d: P(Sofritas | beef=0 + chicken=0 + pork=0 + sea=0) + P(Chick’nitas | beef=0 + * chicken=0 + pork=0 + sea=0) + P(Veggie | beef=0 + chicken=0 + pork=0 + sea=0) > * P(Sofritas | beef>0 or chicken>0 or pork>0 or sea>0) + P(Chick’nitas | beef>0 or * chicken>0 or pork>0 or sea>0) + P(Veggie | beef>0 or chicken>0 or pork>0 or sea>0)
...
rm(dat_h6)
```
Finally, we will explore whether frequent Chipotle customers will be more likely to identify as novel, and therefore select, the Chick’nitas option in arm 3. Specifically, participants who have eaten at a Chipotle more than 3 times in the past 3 months will be more likely to select Chick’nitas than those who have not eaten at a Chipotle in the past 3 months.
H7: P(Chick’nitas | Chipotle>3) > P(Chick’nitas | Chipotle=0)
